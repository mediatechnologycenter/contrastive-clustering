...
do
  ...
  do
    ...
    do
      ...
      time python3 -m src.cc.embedding.embedding_trainer --checkpoint_name "${emb_ckpt}" train_from_file "$(pwd)/data/processed/augmented/augmented_10kgnad-p2p-${data_split}_${aug_fn}.jsonl" --batch_size=8 --num_epoch=2
      time python3 -m src.cc.clustering.cluster_trainer train_from_file agglo "$(pwd)/data/processed/augmented/embeddings/emb_10kgnad-p2p-${data_split}_${aug_fn}_${emb_ckpt}-e2b8.pt"
    done
  done
done