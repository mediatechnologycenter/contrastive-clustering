...
class DataPreprocessor:
    ...
    def rename_content_columns(data: pd.DataFrame, dataset_name: str) -> None:
        ...
        fn_dict = {
            ...
            "tagi": {"first_published_date": "first_published"},
            "20min": {"first_published_date": "first_published"},
            "old-nzz": {"lead_text": "lead",
                        "first_publication_date": "first_published", "publication_date": "published",
                        "publication_last_updated": "last_updated"},
            "old-tagi": {"ts": "first_published"},
        }
    ...
    def convert_to_datetime(data: pd.DataFrame, dataset_name: str) -> pd.DataFrame:
        ...
        fn_dict = {
            "tagi": ["first_published"],  # name after rename
            "20min": ["first_published"],  # name after rename
            "old-20min": ["last_updated", "first_published"],
            "old-tagi": ["first_published"],
            "old-nzz": ["first_published", "published", "last_updated"],
        }
        target_cols = fn_dict.get(dataset_name, list())
    ...
    def remove_html_elements(data: pd.DataFrame, dataset_name: str) -> pd.DataFrame:
        ...
        """
        fn_dict = {
            "tagi": ["title", "lead", "text"],
            "20min": ["title", "lead", "text"],
            "old-nzz": ["title", "lead", "text"],
            "old-tagi": ["title", "lead", "text"],
            "old-20min": ["title", "lead", "text"],
        }
        target_cols = fn_dict.get(dataset_name, list())
        for col in target_cols:
